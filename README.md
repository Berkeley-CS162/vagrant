...
1. Run `vagrant up` to provision the VM locally.
2. SSH in the VM, clear the apt cache, and zero out the disk so that the packaged `.box` file is smaller after compression. That can be done by running `sudo apt clean`, `sudo dd if=/dev/zero of=/EMPTY bs=1M`, and `sudo rm -f /EMPTY` inside the VM. You may also choose to clear the history with `history -c` (and clear the `.bash_history` if it is present). [Here's](https://scotch.io/tutorials/how-to-create-a-vagrant-base-box-from-an-existing-one) a pretty good tutorial. 
3. Run `vagrant halt` to stop the VM and then run `vagrant package --vagrantfile Vagrantfile --output fall2019.box` (choose the correct semester name). 
